...
<script>
	$(document).ready(function () {
			...
			"columns": [
				...
				},
				{
					...
					"render": function (data, type, full) {
						...
						button += '" id="' + full['idtbl_service_order'] + '"><i class="fas fa-pen"></i></button>';

						button += '<a href="#" onclick="approve_confirm(event, \'' + 
							'<?php echo base_url() ?>Serviceorder/Serviceorderstatusapprove/' + 
							full['idtbl_service_order'] + '\')" class="btn btn-outline-info btn-sm mr-1 ';
						if (statuscheck != 1) {
							button += 'd-none';
						}
						button += '" data-toggle="tooltip" data-placement="bottom" title="Approve">' + 
							'<i class="fas fa-check-circle"></i></a>';

						if (full['status'] == 1) {
							button += '<a href="<?php echo base_url() ?>Serviceorder/Serviceorderstatus/' +
		...
		$('#tblserviceorder tbody').on('click', '.btnEdit', async function () {
	...
	function approve_confirm(event, url) {
		event.preventDefault();

		Swal.fire({
			title: "Are you sure?",
			text: "Do you want to approve this service order?",
			icon: "warning",
