...
<div id="layoutSidenav">
					...
					<div class="col-12">
						...
						<div class="scrollbar pb-3 mt-4" id="style-2">
							...
							<table class="table table-bordered table-striped table-sm nowrap"
								...
								<tbody id="tblbodyallocatedmateriallist">
								</tbody>
							</table>
						</div>
					</div>
	...
	<div class="modal fade" id="comepletedeliverymodal" data-backdrop="static" data-keyboard="false" tabindex="-1"
		aria-labelledby="staticBackdropLabel" aria-hidden="true">
		<div class="modal-dialog modal-dialog-centered modal-xl">
			<div class="modal-content">
				<div class="modal-header p-2">
				...
				<div class="modal-body row">
					<div class="col-6">
						<div class="scrollbar pb-3" id="style-2">
							...
							<table class="table table-bordered table-striped table-sm nowrap"
								...
								<tbody id="tblcompletebodyshowdetails">
								</tbody>
							</table>
						</div>
					</div>
					<div class="col-6">
						<div class="scrollbar pb-3" id="style-2">
							<h3>Allocated Materials</h3>
							<table class="table table-bordered table-striped table-sm nowrap"
								id="tblcompleteallocatedmateriallist">
								<thead>
									<tr>
										<th>Material</th>
										<th>Delivery Id</th>
								...
								<tbody id="tblcompletebodyallocatedmateriallist">
								</tbody>
							</table>
						</div>
					</div>
					...
					<div class="col-12 mt-4">
						<div class="scrollbar pb-3" id="style-2">
							<h3>Machine Allocation Summary</h3>
							<table class="table table-bordered table-striped table-sm nowrap"
								id="tblcompletemachinelist">
								<thead>
									<tr>
										<th>Machine</th>
										<th>Delivery Id</th>
								...
								<tbody id="tblbodycompletemachinelist">
								</tbody>
							</table>
						</div>
						<button type="button" id="btnCompleteDelivery" 
							class="btn btn-outline-primary btn-m fa-pull-right mt-4"><i
								class="fas fa-check"></i>&nbsp;Complete Delivery</button>
						<input type="hidden" id="hiddencompletedeliveryid">
					</div>
				</div>
			</div>
		</div>
...
<script>
	$(document).ready(function () {
			...
			"columns": [
				...
				},
				{
					...
					"render": function (data, type, full) {
						...
						if (full['deliverycompletestatus'] == 0) {
							button +=
								'<button class="btn btn-danger btn-sm btnDeliveryView  mr-1" data-status = "' + full['deliverycompletestatus']  + '"id="' +
								full['idtbl_delivery_plan'] +
								'"><i class="fa fa-bus"></i></button>';
						} else {
							button +=
								'<button class="btn btn-success btn-sm btnDeliveryView  mr-1" data-status = "' + full['deliverycompletestatus']  + '"id="' +
								full['idtbl_delivery_plan'] +
								'"><i class="fa fa-bus"></i></button>';
						}
		...
		$(document).on("click", ".btnDeliveryView", function () {
			var id = $(this).attr('id');
			var status = $(this).data("status")

			if(status == 1){
				$('#btnCompleteDelivery').prop("disabled",true)
			}
			$('#hiddencompletedeliveryid').val(id);
			$.ajax({
				type: "POST",
				data: {
					recordID: id
				},
				url: '<?php echo base_url() ?>NewDeliveryPlan/Completedeliveryplanviewjoblist',
				success: function (result) { //alert(result);
					$('#tblcompletebodyshowdetails').html(result);
				}
			});
			$.ajax({
				type: "POST",
				...
				url: '<?php echo base_url() ?>NewDeliveryPlan/Completedeliveryplanviewmateriallist',
				success: function (result) { //alert(result);
					$('#tblcompletebodyallocatedmateriallist').html(result);
				}
			});
			$.ajax({
				type: "POST",
				...
				url: '<?php echo base_url() ?>NewDeliveryPlan/GetMachineAllocationSummary',
				success: function (result) { //alert(result);
					$('#tblbodycompletemachinelist').html(result);
					$('#comepletedeliverymodal').modal('show');
				}
			});
		...
		$(document).on('click', '.btnView', function () {
			...
			$.ajax({
				...
				success: function (result) { //alert(result);
					$('#tblbodyallocatedmateriallist').html(result);
	...
	$('#btnCompleteDelivery').click(function () {
		var id = $('#hiddencompletedeliveryid').val()
		$.ajax({
			type: "POST",
			data: {
				recordId: id,
			},
			url: 'NewDeliveryPlan/CompleteSelectedDelivery',
			success: function (result) { //alert(result);
				console.log(result);
				action(result);
				location.reload()
