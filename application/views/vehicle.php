						...
						<form action="<?php echo base_url() ?>Vehicle/Vehicleinsertupdate" method="post"
							...
							<div class="row">
								...
								<div class="col-3">
									...
									<input type="hidden" name="selected_account_type" id="selected_account_type">
								</div>
							</div>
							<div class="row">
							<div class="col-1">
...
<script>
	$(document).ready(function () {
		...
		$("#chartofdetailaccount").select2({
			width: '100%',
			ajax: {
				url: "<?php echo base_url() ?>Vehicle/Getaccounts",
				type: "post",
				dataType: 'json',
				delay: 250,
		...
		$('#chartofdetailaccount').on('change', function () {
			...
			if (accounttype) {
				$('#selected_account_id').val(chartofdetailaccount);
				$('#selected_account_type').val(accounttype);
			}
		});
				...
				},
				{
					...
					"render": function (data, type, full) {
						...
						button += '<a href="<?php echo base_url() ?>Renewdetails/index/' + full[
								'idtbl_vehicle'] +
							'" target="_self" data-toggle="tooltip" data-placement="bottom" title="Renew Details" class="btn btn-secondary btn-sm mr-1"><i class="fas fa-file"></i></a>';

						if(editcheck==1){
							button+='<button type="button" data-toggle="tooltip" data-placement="bottom" title="Edit" class="btn btn-primary btn-sm btnEdit mr-1" id="'+full['idtbl_vehicle']+'"><i class="fas fa-pen"></i></button>';
						}
						if (full['status'] == 1 && statuscheck == 1) {
							button += '<button type="button" data-toggle="tooltip" data-placement="bottom" title="Active" data-url="Vehicle/Vehiclestatus/' + full['idtbl_vehicle'] + '/2" data-actiontype="2" class="btn btn-success btn-sm mr-1 btntableaction"><i class="fas fa-check"></i></button>';
						} else if (full['status'] != 1 && statuscheck == 1) {
							button += '<button type="button" data-toggle="tooltip" data-placement="bottom" title="Deactive" data-url="Vehicle/Vehiclestatus/' + full['idtbl_vehicle'] + '/1" data-actiontype="1" class="btn btn-warning btn-sm mr-1 btntableaction"><i class="fas fa-times"></i></button>';
						}
						if (deletecheck == 1) {
							button += '<button type="button" data-toggle="tooltip" data-placement="bottom" title="Delete" data-url="Vehicle/Vehiclestatus/' + full['idtbl_vehicle'] + '/3" data-actiontype="3" class="btn btn-danger btn-sm btntableaction"><i class="fas fa-trash-alt"></i></button>';
						}
