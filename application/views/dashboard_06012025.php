...
$sql="SELECT m.materialname, g.date, g.qty, g.unitprice, g.total 
	  ...
	  WHERE g.status = 1 AND gd.approvestatus=1 AND gd.company_id=$company AND g.tbl_print_material_info_idtbl_print_material_info <> 0 ORDER BY g.date DESC LIMIT 5";
$resultdate = $this->db->query($sql);

// Top Five Purchase Report
$sql="SELECT m.materialname, g.date, g.qty, g.unitprice, g.total FROM tbl_print_grndetail g LEFT JOIN tbl_print_grn gd ON gd.idtbl_print_grn=g.tbl_print_grn_idtbl_print_grn INNER JOIN tbl_print_material_info m ON g.tbl_print_material_info_idtbl_print_material_info = m.idtbl_print_material_info WHERE g.status = 1 AND gd.approvestatus=1 AND gd.company_id=$company AND g.tbl_print_material_info_idtbl_print_material_info <> 0 ORDER BY g.qty DESC LIMIT 5 ";
