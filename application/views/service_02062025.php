				...
				},
				{
					...
					"render": function (data, type, full) {
						...
						button += '<a href="<?php echo base_url() ?>Servicesitem/index/' + full[
								'idtbl_service'] +
							'" target="_self" class="btn btn-dark btn-sm mr-1"><i class="fas fa-plus"></i></a>';

							if(editcheck==1){
								button+='<button type="button" class="btn btn-primary btn-sm btnEdit mr-1" id="'+full['idtbl_service']+'"><i class="fas fa-pen"></i></button>';
							}
							if (full['status'] == 1 && statuscheck == 1) {
								button += '<button type="button" data-url="Service/servicestatus/' + full['idtbl_service'] + '/2" data-action="deactivate" class="btn btn-success btn-sm mr-1 btntableaction"><i class="fas fa-check"></i></button>';
							} else if (full['status'] != 1 && statuscheck == 1) {
								button += '<button type="button" data-url="Service/servicestatus/' + full['idtbl_service'] + '/1" data-action="activate" class="btn btn-warning btn-sm mr-1 btntableaction"><i class="fas fa-times"></i></button>';
							}

							if (deletecheck == 1) {
								button += '<button type="button" data-url="Service/servicestatus/' + full['idtbl_service'] + '/3" data-action="delete" class="btn btn-danger btn-sm btntableaction"><i class="fas fa-trash-alt"></i></button>';
							}
