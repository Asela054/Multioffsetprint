...
<script>
/* Injects a heading into every accordion submenu panel, taken from its own
   trigger link's label (e.g. "Master Information"). Only used visually when
   the sidebar is collapsed to a rail (see .flyout-heading in custom-theme.css) -
   it is inert/invisible in the normal expanded layout, so nothing existing changes. */
(function () {
	function addFlyoutHeadings() {
		document.querySelectorAll('#accordionSidenav .nav-link[data-toggle="collapse"]').forEach(function (trigger) {
			var targetSel = trigger.getAttribute('data-target');
			if (!targetSel) return;
			var panel = document.querySelector(targetSel);
			if (!panel || panel.querySelector(':scope > .flyout-heading')) return;

			var span = trigger.querySelector('span');
			var label = span ? span.textContent.trim() : trigger.textContent.trim();
			if (!label) return;

			var heading = document.createElement('div');
			heading.className = 'flyout-heading';
	...
	if (document.readyState === 'loading') {
		document.addEventListener('DOMContentLoaded', addFlyoutHeadings);
	} else {
		addFlyoutHeadings();
	}
})();
