...
class Materialdetailinfo extends CI_Model{
        ...
        foreach ($uom_options as $uom_id) {
            ...
            if ($uom_details) {
                $material_qty_data = [
                    ...
                    'insertdatetime' => $updatedatetime,
                    'tbl_user_idtbl_user' => $userID,
                    'tbl_measurements_idtbl_measurements' => $uom_details['main_uom'],
                ];
    ...
    {
        $this->db->select('idtbl_uom_conversions, main.measure_type AS main_uom, convert.measure_type AS convert_uom, qty');
        $this->db->from('tbl_uom_conversions');
        $this->db->join('tbl_measurements as main', 'main.idtbl_mesurements = tbl_uom_conversions.main_uom', 'left');
        $this->db->join('tbl_measurements as convert', 'convert.idtbl_mesurements = tbl_uom_conversions.convert_uom', 'left');
        $this->db->where('tbl_uom_conversions.status', 1);
    
        $query = $this->db->get();
    
        if ($query->num_rows() > 0) {
            $data = $query->result_array();
            echo json_encode(['status' => 'success', 'uom_conversions' => $data]);
        } else {
