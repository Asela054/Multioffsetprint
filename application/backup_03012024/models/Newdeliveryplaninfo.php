...
class Newdeliveryplaninfo extends CI_Model{
    ...
    public function Deliveryplanviewmateriallist(){
		...
		$sql="SELECT `tbl_delivery_plan_material_allocation`.`idtbl_delivery_plan_material_allocation`, `tbl_delivery_plan_material_allocation`.`qty`, `tbl_delivery_plan_details`.`special_id`, `tbl_print_material_info`.`materialname`, `tbl_print_material_info`.`materialinfocode` FROM `tbl_delivery_plan_details` JOIN `tbl_delivery_plan` ON `tbl_delivery_plan`.`idtbl_delivery_plan`=`tbl_delivery_plan_details`.`tbl_delivery_plan_idtbl_delivery_plan` JOIN `tbl_delivery_plan_material_allocation` ON (`tbl_delivery_plan_material_allocation`.`tbl_delivery_plan_details_idtbl_delivery_plan_details` = `tbl_delivery_plan_details`.`idtbl_delivery_plan_details`) JOIN `tbl_print_material_info` ON (`tbl_print_material_info`.`idtbl_print_material_info` = `tbl_delivery_plan_material_allocation`.`tbl_print_material_info_idtbl_print_material_info`) WHERE `tbl_delivery_plan_idtbl_delivery_plan`= '$recordID'";
    ...
    public function AllocationInsertUpdate(){
        ...
        foreach($tableData as $rowtabledata){
            ...
            while($batchqty < $reducedqty){
                ...
                $this->db->where('tbl_print_material_info_idtbl_print_material_info', $materialID);
                $this->db->update('tbl_print_stock', $data);
    ...
    public function Completedeliveryplanviewjoblist(){
		...
		$sql="SELECT `tbl_delivery_plan_details`.`idtbl_delivery_plan_details`, `tbl_customerinquiry_detail`.`job`, `tbl_delivery_plan_details`.`special_id`, `tbl_delivery_plan_details`.`deliveryDate`, `tbl_delivery_plan_details`.`qty`, `tbl_delivery_plan`.`deliverycompletestatus` FROM `tbl_delivery_plan_details` LEFT JOIN `tbl_delivery_plan` ON `tbl_delivery_plan`.`idtbl_delivery_plan`=`tbl_delivery_plan_details`.`tbl_delivery_plan_idtbl_delivery_plan` JOIN `tbl_customerinquiry_detail` ON (`tbl_customerinquiry_detail`.`idtbl_customerinquiry_detail` = `tbl_delivery_plan_details`.`tbl_customerinquiry_detail_idtbl_customerinquiry_detail`) WHERE `tbl_delivery_plan_idtbl_delivery_plan`= '$recordID'";
        ...
        foreach($respond->result() as $rowlist){
            ...
            <tr id ="'.$rowlist->idtbl_delivery_plan_details.'">
                ...
                if($rowlist->deliverycompletestatus == 1){
                    $html .= '<td class = "text-success">Completed</td>';
                }else{
                    $html .= '<td class = "text-danger">Not Completed</td>';
    ...
    public function GetMachineAllocationSummary(){
		...
		$sql="SELECT `ue`.`idtbl_delivery_plan_details`, `u`.`machine`, `ue`.`special_id`, `ub`.`startdatetime`, `ub`.`enddatetime`, `ub`.`completed_status`, SUM(`uc`.`completedqty`) AS completedqty_sum, SUM(`uc`.`wastageqty`) AS wastageqty_sum FROM `tbl_machine` AS `u` JOIN `tbl_machine_allocation` AS `ub` ON (`u`.`idtbl_machine` = `ub`.`tbl_machine_idtbl_machine`) LEFT JOIN `tbl_machine_allocation_details` AS `uc` ON (`ub`.`idtbl_machine_allocation` = `uc`.`tbl_machine_allocation_idtbl_machine_allocation`) JOIN `tbl_cost_items` AS `ud` ON (`ud`.`idtbl_cost_items` = `ub`.`tbl_cost_items_idtbl_cost_items`) JOIN `tbl_delivery_plan_details` AS `ue` ON (`ue`.`idtbl_delivery_plan_details` = `ub`.`tbl_delivery_plan_details_idtbl_delivery_plan_details`) WHERE `ue`.`tbl_delivery_plan_idtbl_delivery_plan` = '$recordID' AND `ub`.`status` IN (1) GROUP BY `ub`.`idtbl_machine_allocation`";
        ...
        foreach($respond->result() as $rowlist){
            ...
            <tr id ="'.$rowlist->idtbl_delivery_plan_details.'">
                ...
                <td>'.$rowlist->completedqty_sum.'</td>
                <td>'.$rowlist->wastageqty_sum.'</td>';
            if($rowlist->completed_status == 1){
                $html.='<td class = "text-success">Completed</td>';
            } else{
                $html.='<td class = "text-danger">Not Completed</td>';
            }
