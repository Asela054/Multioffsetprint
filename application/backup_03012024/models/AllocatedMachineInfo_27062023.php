...
class AllocatedMachineInfo extends CI_Model{
    ...
    public function GetAllocationData(){
        ...
        // LEFT JOIN `tbl_cost_items` AS `uc` ON `ub`.`idtbl_customerinquiry_detail` = `uc`.`tbl_customerinquiry_detail_idtbl_customerinquiry_detail` LEFT JOIN `tbl_customer` AS `ud` ON (`ud`.`idtbl_customer` = `u`.`tbl_customer_idtbl_customer`) WHERE `u`.`status` IN (1) AND `ub`.`status` IN (1) AND `uc`.`status` IN (1) AND `uc`.`tbl_customerinquiry_detail_idtbl_customerinquiry_detail` = '$recordID'";

        $sql = "SELECT * FROM `tbl_machine_allocation` AS `ma` LEFT JOIN `tbl_machine` AS `m` ON (`m`.`idtbl_machine` = `ma`.`tbl_machine_idtbl_machine`) LEFT JOIN `tbl_cost_items` AS `ci` ON (`ci`.`idtbl_cost_items` = `ma`.`tbl_cost_items_idtbl_cost_items`) LEFT JOIN `tbl_customerinquiry_detail` AS `d` ON (`d`.`idtbl_customerinquiry_detail` = `ci`.`tbl_customerinquiry_detail_idtbl_customerinquiry_detail`) LEFT JOIN `tbl_customerinquiry` AS `i` ON (`i`.`idtbl_customerinquiry` = `d`.`tbl_customerinquiry_idtbl_customerinquiry`) LEFT JOIN `tbl_customer` AS `c` ON (`c`.`idtbl_customer` = `i`.`tbl_customer_idtbl_customer`) WHERE `ma`.`tbl_machine_idtbl_machine` = '$recordID' ORDER BY `ma`.`startdatetime` ASC";
    ...
    public function GetAllocationHours(){
        ...
        // $this->db->select('startdatetime, name');
