<?php class Newpurchaserequestinfo extends CI_Model {
	...
	public function Getstockqty() {
		...
		$sql = "SELECT `qty` FROM `tbl_print_stock` LEFT JOIN `tbl_print_material_info` ON `tbl_print_material_info`.`idtbl_print_material_info`=`tbl_print_stock`.`tbl_print_material_info_idtbl_print_material_info` WHERE `tbl_print_stock`.`status`=1 AND `materialname`=? AND `tbl_print_stock`.`company_id`=? AND `tbl_print_stock`.`company_branch_id`=?";
