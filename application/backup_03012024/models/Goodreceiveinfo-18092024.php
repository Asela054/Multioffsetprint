	...
	public function Goodreceiveinsertupdate() {
		...
		$date1 = $this->input->post('grndate');
            ...
            if ($query->num_rows() > 0) {
                ...
                $grn_number = intval(substr($last_grn_no, -4)); // Extract last 4 digits (example 0808 from POR230808)********************
                $count = $grn_number;
            } else {
                $count = 0;
            }
		...
		$result_array = array();
		$querydetails = $this->db->query("SELECT idtbl_print_grn FROM tbl_print_grn WHERE idtbl_print_grn = $grnID");
		if ($querydetails) {
		...
		foreach ($result_array as $row) {
			...
			$this->db->where('idtbl_print_grn', $id);
