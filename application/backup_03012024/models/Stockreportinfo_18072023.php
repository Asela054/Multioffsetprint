...
class Stockreportinfo extends CI_Model{
    ...
    public function Getproductaccosupplier(){
        ...
        $sql="SELECT `idtbl_material_info`, `materialinfocode`, `materialname` FROM `tbl_material_info` WHERE `tbl_supplier_idtbl_supplier`=?";
    ...
    public function Purchaseorderview(){
        ...
        $sql="SELECT `u`.*, `ua`.`name`, `ua`.`address_line1`, `ub`.`location`, `ub`.`phone`, `ub`.`address`, `ub`.`phone2`, `ub`.`email` AS `locemail` FROM `tbl_porder` AS `u` LEFT JOIN `tbl_supplier` AS `ua` ON (`ua`.`idtbl_supplier` = `u`.`tbl_supplier_idtbl_supplier`) LEFT JOIN `tbl_location` AS `ub` ON (`ub`.`idtbl_location` = `u`.`tbl_location_idtbl_location`) WHERE `u`.`status`=? AND `u`.`idtbl_porder`=?";
