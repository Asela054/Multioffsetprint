<?php class Newpurchaserequestinfo extends CI_Model {
	...
	public function Getstockqty() {
		...
		$sql = "SELECT `qty` FROM `tbl_print_stock` WHERE `status`=1 AND `tbl_print_material_info_idtbl_print_material_info`=? AND `company_id`=? AND `company_branch_id`=?";
	...
	public function Getpiecesforqty() {
		...
		if ($result) {
			$response->piecesper_qty = $result->qty*$qty;  // Set the retrieved quantity
		} else {
	...
	public function Newpurchaserequestinsertupdate() {
		...
		$date1 = $this->input->post('insertdatetime');
            ...
            if ($query->num_rows() > 0) {
                ...
                $req_number = intval(substr($last_req_no, -4)); // Extract last 4 digits (example 0808 from POR230808)********************
                $count = $req_number;
            } else {
                $count = 0;
            }
	...
	public function Purchaseorderview() {
		...
		$this->db->select('tbl_print_porder_req_detail.*,tbl_print_porder_req.tbl_order_type_idtbl_order_type, tbl_print_material_info.materialinfocode, tbl_print_material_info.materialname, tbl_spareparts.spare_part_name, tbl_machine_type.type,tbl_machine.machine,tbl_machine.machinecode, tbl_service_type.service_name,tbl_measurements.measure_type ');
		...
		$this->db->join('tbl_spareparts', 'tbl_spareparts.idtbl_spareparts = tbl_print_porder_req_detail.tbl_sparepart_id', 'left');
		$this->db->join('tbl_machine_type', 'tbl_machine_type.idtbl_machine_type = tbl_spareparts.machine_type_id', 'left');
        ...
        <td>'.$roworderinfo->machine.'/ '.$roworderinfo->machinecode.'</td><td>'.$roworderinfo->qty.'</td><td>'.$roworderinfo->measure_type.'</td></tr>';
				...
				$html.='<tr>
        <td>'.$roworderinfo->spare_part_name.'/ '.$roworderinfo->type.'</td><td>'.$roworderinfo->qty.'</td><td>'.$roworderinfo->measure_type.'</td></tr>';
