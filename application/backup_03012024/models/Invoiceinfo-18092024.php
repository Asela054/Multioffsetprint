<?php
    ...
    public function Getcustomerlist() {
        ...
        $this->db->where('tbl_customer.company_id', $comapnyID);
        ...
        public function Invoiceinsertupdate(){
            ...
            //  $currentYear = date('Y'); //2024-03-31= (from 2023-04-01 to 2024-03-31)  || 2024-05-01= (from 2024-04-01 to 2025-03-31)
            //  $currenMonth = date('m'); //03
            ...
            $query = $this->db->query("SELECT inv_no FROM tbl_print_invoice WHERE company_id = $comapnyID AND `date` BETWEEN '$fromyear' AND '$toyear' ORDER BY inv_no DESC LIMIT 1");
            
            if ($query->num_rows() > 0) {
                $last_inv_no = $query->row()->inv_no;
           ...
           //$ponum=$this->input->post('ponum');
            ...
            foreach ($result_array as $row) {
                ...
                $invono = 'INV' . $yearDigit . $countPrefix;


                // $invono = 'INV' . $countPrefix;
...
echo $html;
        ...
        foreach($tableData as $rowtabledata) {
			...
			$this->db->select('tbl_print_invoice.idtbl_print_invoice,tbl_print_invoice.date, tbl_print_invoice.total,tbl_print_invoice.customer_id,tbl_print_invoice.inv_no,tbl_print_invoice.vat_amount,tbl_print_invoice.subtotal,tbl_customer.vat_customer');
			$this->db->from('tbl_print_invoice');
            $this->db->join('tbl_customer', 'tbl_print_invoice.customer_id = tbl_customer.idtbl_customer');
			...
			if ($respond->num_rows() > 0) {
				foreach ($respond->result() as $row) {
                    ...
                    // Initialize both statuses with default values
                        // $svat_status = '0';
                        // $vat_status = '0';

                        // // Conditional assignment based on $customer_type
                        // if ($customer_type == 2) {
                        //     $svat_status = '1';
						...
						$accountsData = array(
							...
							'amount' => $totalamount,
							'status' => '1',
                            // 'svat_status' => $svat_status,  
                            // 'vat_status' => $vat_status,    
							'insertdatetime' => $updatedatetime,
							'tbl_user_idtbl_user' => $userID
						);
						// Insert the data into the 'tbl_sales_info' table
            ...
            if ($customer_type == 0 || $customer_type == 1) {
                $segregation_data = [
                    [
                        "amount" => $vatamount, // Amount for the Vat reserve chartaccount 
                        "narration" => $invoicenum . ' - ' . $invoicedate,
                        "chartaccount" => "141",
                        "detailaccount" => ""
                    ],
                    [
                        "amount" => $grossamount, // Amount for the Sales job detailaccount
                        "narration" => $invoicenum . ' - ' . $invoicedate,
                        "chartaccount" => "",
                        "detailaccount" => "1"
                    ]
                ...
                $segregation_data = [
                    [
                        "amount" => $totalamount, // Amount for the Sales Jobs - BOI detailaccount
                        "narration" => $invoicenum . ' - ' . $invoicedate,
                        "chartaccount" => "",
                        "detailaccount" => "2"
                    ]
            ...
            $data = [
                ...
                "segregationdata" => json_encode($segregation_data)
            ];
            
            $postfields = http_build_query($data);
    
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, "https://aws.erav.lk/multioffsetaccount/Api/Receiptsegregationinsertupdate");
            ...
            if ($curl_errno == 0) {
                ...
                if ($this->db->trans_status() === TRUE) {
                    ...
                    $actionObj->icon = 'fas fa-save';
                    $actionObj->title = '';
                    $actionObj->message = 'Invoice Confirmed Successfully';
                    $actionObj->url = '';
                    $actionObj->target = '_blank';
                    $actionObj->type = 'success';
