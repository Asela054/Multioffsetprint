<?php class Purchaseorderinfo extends CI_Model {
	...
	public function Getproductinfoaccoproduct() {
		...
		$this->db->select('qty, unitprice, measure_type_id, pieces');
	...
	public function Purchaseorderinsertupdate() {
		...
		$date1 = $this->input->post('orderdate');
            ...
            if ($query->num_rows() > 0) {
                ...
                $po_number = intval(substr($last_req_no, -4)); // Extract last 4 digits (example 0808 from POR230808)********************
                $count = $po_number;
            } else {
                $count = 0;
            }
