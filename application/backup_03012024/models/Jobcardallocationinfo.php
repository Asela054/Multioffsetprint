...
class Jobcardallocationinfo extends CI_Model{
    ...
    public function GetNonValidatedAccounts() {
        ...
        $this->db->select('`tbl_print_stock.batchno`, `tbl_print_grn`.`voucherissue`');
        $this->db->from('tbl_jobcard_material');
		$this->db->join('tbl_print_stock', 'tbl_print_stock.idtbl_print_stock = tbl_jobcard_material.tbl_print_stock_idtbl_print_stock');
		$this->db->join('tbl_print_grn', 'tbl_print_grn.batchno = tbl_print_stock.batchno');
        $this->db->where('tbl_jobcard_material.tbl_jobcard_idtbl_jobcard', $recordId);
        $this->db->group_by('`tbl_print_stock.batchno`');
        $query1 = $this->db->get_compiled_select();

        $this->db->select('`tbl_print_stock.batchno`, `tbl_print_grn`.`voucherissue`');
        $this->db->from('tbl_jobcard_lamination');
		$this->db->join('tbl_print_stock', 'tbl_print_stock.idtbl_print_stock = tbl_jobcard_lamination.tbl_print_stock_idtbl_print_stock');
		$this->db->join('tbl_print_grn', 'tbl_print_grn.batchno = tbl_print_stock.batchno');
		...
		$this->db->join('tbl_print_stock', 'tbl_print_stock.idtbl_print_stock = tbl_jobcard_rimming.tbl_print_stock_idtbl_print_stock');
		$this->db->join('tbl_print_grn', 'tbl_print_grn.batchno = tbl_print_stock.batchno');
		...
		$this->db->join('tbl_print_stock', 'tbl_print_stock.idtbl_print_stock = tbl_jobcard_varnish.tbl_print_stock_idtbl_print_stock');
		$this->db->join('tbl_print_grn', 'tbl_print_grn.batchno = tbl_print_stock.batchno');
    ...
    public function IssueStock($x) {
        ...
        $this->db->select('`sheetqty`, `tbl_print_stock_idtbl_print_stock`, `qty`');
		$this->db->from('tbl_jobcard');
		$this->db->join('tbl_jobcard_material', 'tbl_jobcard.idtbl_jobcard = tbl_jobcard_material.tbl_jobcard_idtbl_jobcard');
		$this->db->join('tbl_print_stock', 'tbl_print_stock.idtbl_print_stock = tbl_jobcard_material.tbl_print_stock_idtbl_print_stock');
		$this->db->where('tbl_jobcard.idtbl_jobcard', $recordID);
        ...
        $this->db->select('`varnishQty`, `tbl_print_stock_idtbl_print_stock`, `qty`');
		$this->db->from('tbl_jobcard');
		$this->db->join('tbl_jobcard_varnish', 'tbl_jobcard.idtbl_jobcard = tbl_jobcard_varnish.tbl_jobcard_idtbl_jobcard');
		$this->db->join('tbl_print_stock', 'tbl_print_stock.idtbl_print_stock = tbl_jobcard_varnish.tbl_print_stock_idtbl_print_stock');
		$this->db->where('tbl_jobcard.idtbl_jobcard', $recordID);
        ...
        $this->db->select('`lamination_qty`, `tbl_print_stock_idtbl_print_stock`, `qty`');
		$this->db->from('tbl_jobcard');
		$this->db->join('tbl_jobcard_lamination', 'tbl_jobcard.idtbl_jobcard = tbl_jobcard_lamination.tbl_jobcard_idtbl_jobcard');
		$this->db->join('tbl_print_stock', 'tbl_print_stock.idtbl_print_stock = tbl_jobcard_lamination.tbl_print_stock_idtbl_print_stock');
		$this->db->where('tbl_jobcard.idtbl_jobcard', $recordID);
        ...
        $this->db->select('tbl_jobcard_rimming.qty AS rimming_qty, tbl_print_stock.qty AS stock_qty, tbl_jobcard_rimming.tbl_print_stock_idtbl_print_stock');
		$this->db->from('tbl_jobcard');
		$this->db->join('tbl_jobcard_rimming', 'tbl_jobcard.idtbl_jobcard = tbl_jobcard_rimming.tbl_jobcard_idtbl_jobcard');
		$this->db->join('tbl_print_stock', 'tbl_print_stock.idtbl_print_stock = tbl_jobcard_rimming.tbl_print_stock_idtbl_print_stock');
		$this->db->where('tbl_jobcard.idtbl_jobcard', $recordID);
        ...
        try {
            foreach ($materialdetail as $materialrow) {
                $qty = $materialrow->sheetqty;
                $stockqty = $materialrow->qty;
                $stockId = $materialrow->tbl_print_stock_idtbl_print_stock;
            ...
            foreach ($varnishdetail as $varnishrow) {
                $qty = $varnishrow->varnishQty;
                $stockqty = $materialrow->qty;
                $stockId = $varnishrow->tbl_print_stock_idtbl_print_stock;

                if ($qty > $stockqty) {
                    throw new Exception("Insufficient stock for Varnish ID: {$stockId}");
                }
            ...
            foreach ($laminationdetail as $laminationrow) {
                $qty = $laminationrow->lamination_qty;
                $stockqty = $materialrow->qty;
                $stockId = $laminationrow->tbl_print_stock_idtbl_print_stock;

                if ($qty > $stockqty) {
                    throw new Exception("Insufficient stock for Lamination ID: {$stockId}");
                }
            ...
            foreach ($rimmingdetails as $rimmingrow) {
                $qty = $rimmingrow->rimming_qty;
                $stockqty = $rimmingrow->stock_qty;
                $stockId = $rimmingrow->tbl_print_stock_idtbl_print_stock;

                if ($qty > $stockqty) {
                    throw new Exception("Insufficient stock for Rimming ID: {$stockId}");
                }
    ...
    public function TransferToAccounts() {
        ...
        $this->db->select('SUM(tbl_jobcard_material.sheetqty * tbl_print_stock.unitprice) AS total_price');
        $this->db->from('tbl_jobcard_material');
        $this->db->join('tbl_print_stock', 'tbl_print_stock.idtbl_print_stock = tbl_jobcard_material.tbl_print_stock_idtbl_print_stock');
        $this->db->where('tbl_jobcard_material.tbl_jobcard_idtbl_jobcard', $recordID);
        $querymaterial = $this->db->get();
        $materialTot = $querymaterial->row()->total_price;
       
       
        $this->db->select('SUM(tbl_jobcard_varnish.varnishQty * tbl_print_stock.unitprice) AS total_price');
		$this->db->from('tbl_jobcard_varnish');
		$this->db->join('tbl_print_stock', 'tbl_print_stock.idtbl_print_stock = tbl_jobcard_varnish.tbl_print_stock_idtbl_print_stock');
		$this->db->where('tbl_jobcard_varnish.tbl_jobcard_idtbl_jobcard', $recordID);
        $quueryvarnish = $this->db->get();
        $varnishTot = $quueryvarnish->row()->total_price;      
      
        $this->db->select('SUM(tbl_jobcard_lamination.lamination_qty * tbl_print_stock.unitprice) AS total_price');
		$this->db->from('tbl_jobcard_lamination');
		$this->db->join('tbl_print_stock', 'tbl_print_stock.idtbl_print_stock = tbl_jobcard_lamination.tbl_print_stock_idtbl_print_stock');
		$this->db->where('tbl_jobcard_lamination.tbl_jobcard_idtbl_jobcard', $recordID);
        $querylamination = $this->db->get();
        $laminationTot = $querylamination->row()->total_price;    
    
        $this->db->select('SUM(tbl_jobcard_rimming.qty * tbl_print_stock.unitprice) AS total_price');
		$this->db->from('tbl_jobcard_rimming');
		$this->db->join('tbl_print_stock', 'tbl_print_stock.idtbl_print_stock = tbl_jobcard_rimming.tbl_print_stock_idtbl_print_stock');
		$this->db->where('tbl_jobcard_rimming.tbl_jobcard_idtbl_jobcard', $recordID);
        $queryrimming = $this->db->get();
        $rimmingTot = $queryrimming->row()->total_price;
        
        $fullTot = $materialTot + $varnishTot + $laminationTot + $rimmingTot;
