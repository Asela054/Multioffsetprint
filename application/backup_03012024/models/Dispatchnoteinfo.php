...
class Dispatchnoteinfo extends CI_Model{
	...
	public function Dispatchnoteinsertupdate(){
		...
		$query = $this->db->query("SELECT dispatch_no FROM tbl_print_dispatch WHERE company_id = $comapnyID AND `date` BETWEEN '$fromyear' AND '$toyear' ORDER BY dispatch_no DESC LIMIT 1");
		
		if ($query->num_rows() > 0) {
			$last_dis_no = $query->row()->dispatch_no;
			$dis_number = intval(substr($last_dis_no, -4)); // Extract last 4 digits (example 0808 from INV230808)********************
			$count = $dis_number;
		} else {
			$count = 0;
		}
		...
		$result_array = array();
            $querydetails = $this->db->query("SELECT idtbl_print_dispatch FROM tbl_print_dispatch WHERE idtbl_print_dispatch = $dispatchID");
            if ($querydetails) {
                $result_array = $querydetails->result_array();
            }
    
            foreach ($result_array as $row) {
                $id = $row['idtbl_print_dispatch'];
                ...
                $disno = 'DPN' . $yearDigit . $countPrefix;
                ...
                $this->db->where('idtbl_print_dispatch', $id);
                $this->db->update('tbl_print_dispatch', $invodetail);
            }
		// $materialid=0;
		// $machineid=0;
