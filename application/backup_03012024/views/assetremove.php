        	...
        	<div class="modal-dialog modal-lg  modal-dialog-centered" role="document">
        		<div class="modal-content">
        			...
        			<div class="modal-body">
                        <form action="<?php echo base_url() ?>Assetremove/Assetremoveinsertupdate" method="post" autocomplete="off">
                        	<div class="row">
                        			<div class="col-4">
                            ...
                            <div class="row">
                        			<div class="col-4">
                                        ...
                                        <select class="form-control form-control-sm" name="remove_type" id="remove_type" required>
                        			...
                        			<div class="col-4">
                        				<div class="form-group mb-1">
                        					...
                        					<input type="number" step="any" class="form-control form-control-sm"
                        						name="currentvalue" id="currentvalue" required>
                        				</div>
                        			</div>
                        			<div class="col-4">
                        				<div class="form-group mb-1">
...
<script>
    $(document).ready(function() {
        ...
        $('#tblremoved').DataTable({
            ...
            "buttons": [
                { extend: 'csv', className: 'btn btn-success btn-sm', title: 'Removed Asset Information', text: '<i class="fas fa-file-csv mr-2"></i> CSV', },
                { extend: 'pdf', className: 'btn btn-danger btn-sm', title: 'Removed Asset Information', text: '<i class="fas fa-file-pdf mr-2"></i> PDF', },
                { 
                    extend: 'print', 
                    title: 'Removed Asset Information',
                    className: 'btn btn-primary btn-sm', 
                    text: '<i class="fas fa-print mr-2"></i> Print',
                    customize: function ( win ) {
                        $(win.document.body).find( 'table' )
            ...
            "columns": [
                ...
                },
                {
                    ...
                    "render": function(data, type, full) {
                        var button='';
                        button+='<button class="btn btn-primary btn-sm btnEdit mr-1 ';if(editcheck!=1){button+='d-none';}button+='" id="'+full['idtbl_asset_remove']+'"><i class="fas fa-pen"></i></button>';
                        if(full['status']==1){
                            button+='<a href="<?php echo base_url() ?>Assetremove/Assetremovestatus/'+full['idtbl_asset_remove']+'/2" onclick="return deactive_confirm()" target="_self" class="btn btn-success btn-sm mr-1 ';if(statuscheck!=1){button+='d-none';}button+='"><i class="fas fa-check"></i></a>';
                        }else{
                            button+='<a href="<?php echo base_url() ?>Assetremove/Assetremovestatus/'+full['idtbl_asset_remove']+'/1" onclick="return active_confirm()" target="_self" class="btn btn-warning btn-sm mr-1 ';if(statuscheck!=1){button+='d-none';}button+='"><i class="fas fa-times"></i></a>';
                        }
                        button+='<a href="<?php echo base_url() ?>Assetremove/Assetremovestatus/'+full['idtbl_asset_remove']+'/3" onclick="return delete_confirm()" target="_self" class="btn btn-danger btn-sm ';if(deletecheck!=1){button+='d-none';}button+='"><i class="fas fa-trash-alt"></i></a>';
                         return button;
                    }
                }
            ],
        ...
        $('#tblremoved tbody').on('click', '.btnEdit', function() {
                ...
                $.ajax({
                    ...
                    url: '<?php echo base_url() ?>Assetremove/Assetremoveedit',
