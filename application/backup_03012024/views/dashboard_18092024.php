...
$sql="SELECT m.materialname, g.grndate, g.qty, g.unitprice, g.total FROM tbl_print_stock g INNER JOIN tbl_print_material_info m ON g.tbl_print_material_info_idtbl_print_material_info = m.idtbl_print_material_info WHERE g.status = 1 AND g.tbl_print_material_info_idtbl_print_material_info <> 0 AND (g.updatedatetime IS NULL OR g.updatedatetime > g.grndate) <> 0 ORDER BY g.qty DESC LIMIT 5";
