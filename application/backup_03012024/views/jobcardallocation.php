													...
													<div class="form-row">
														<div class="col mt-2">
															...
															<select name="paperboard" id="paperboard"
																...
																<option
																	value="<?php echo $rowmaterial->idtbl_print_stock ?>"
																	data-stockid="<?php echo $rowmaterial->idtbl_print_stock ?>"
																	data-stockqty="<?php echo $rowmaterial->qty ?>">
																	<?php echo $rowmaterial->materialname ?> /BTC:
																	<?php echo $rowmaterial->batchno ?> /Qty:
																	<?php echo $rowmaterial->qty ?>
																</option>
																<?php } ?>
															</select>
														</div>
	...
	$(document).ready(function () {
		...
		$('#btnMaterialSubmit').click(function () {
			if (!$("#materialform")[0].checkValidity()) {
				...
				var cutmaterialtext = $("#cutmaterial option:selected").text();

				var stockId = $('#paperboard').find(':selected').data('stockid');
				var stockqty = $('#paperboard').find(':selected').data('stockqty');

				var cutsize = $('#cutsize').val();
				var paperSheetsQty = $('#paperSheetsQty').val();

				if (paperSheetsQty > stockqty) {
					alert('Not enough stock available in this Batch');
					return;
				}
				...
				$('#tblMaterialDetails > tbody:last').append('<tr><td class="d-none">' +
					...
					paperSheetsQty + '</td><td><button type="button" id="' +
					materialId +
		...
		$('#btnVarnishSubmit').click(function () {
			if (!$("#varnishform")[0].checkValidity()) {
				...
				var stockId = $('#varnishmaterial').find(':selected').data('stockid');
				var stockqty = $('#varnishmaterial').find(':selected').data('stockqty');

				if (varnishSheetsQty > stockqty) {
					alert('Not enough stock available in this Batch');
					return;
				}
		...
		$('#btnLaminationSubmit').click(function () {
			if (!$("#laminationform")[0].checkValidity()) {
				...
				var stockId = $('#laminatingmaterial').find(':selected').data('stockid');
				var stockqty = $('#laminatingmaterial').find(':selected').data('stockqty');

				if (laminationSheets > stockqty) {
					alert('Not enough stock available in this Batch');
					return;
				}
		...
		$('#btnRimmingSubmit').click(function () {
			if (!$("#rimmingForm")[0].checkValidity()) {
				...
				var stockId = $('#rimmingmaterial').find(':selected').data('stockid');
				var stockqty = $('#rimmingmaterial').find(':selected').data('stockqty');

				if (rimmingsheets > stockqty) {
					alert('Not enough stock available in this Batch');
					return;
				}
				...
				success: function (result) { //alert(result);
					...
					$.each(obj, function (i, item) {
						...
						$('#BatchNoTbl > tbody:last').append(
							...
							'</td><td class="text-center ' + (obj[i].voucherissue == 0 ?
