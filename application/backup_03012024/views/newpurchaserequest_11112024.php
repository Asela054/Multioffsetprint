                                        ...
                                        <div class="col-sm-12 col-md-12 col-lg-4 col-xl-4">
                                            <form id="createorderform" autocomplete="off">
                                                ...
                                                <div class="form-row mb-1">
                                                    ...
                                                    <div class="col">
                                                        <label class="small font-weight-bold text-dark">No. of pieces (Sheets)</label>
                                                ...
                                                <div class="form-group mb-1">
                                                    ...
                                                    <label class="small font-weight-bold text-dark d-none">Unit Price </label>
                                                    <input type="number" id="unitprice" name="unitprice"
                                                        class="form-control form-control-sm d-none" value="0" step="any">
                                                </div>
...
$(document).ready(function() {
    ...
    $('#uom').change(function() {
        let uomID = $(this).val();  // Get selected UOM ID
        let productId = $('#product').val();  // Assuming there's an element with the ID 'product' for product ID
        let qty = $('#newqty').val();  // Get the entered quantity

        $.ajax({
            type: "POST",
            url: 'Newpurchaserequest/Getpiecesforqty',
            data: {
                recordID: uomID,
                productId: productId,
                qty: qty
            ...
            success: function(result) {
                var obj = JSON.parse(result);
                $('#piecesper_qty').val(obj.piecesper_qty);  // Set the "No. of pieces (Sheets)" field
            }
        });
    });

    // Optional: Trigger UOM calculation on quantity change as well
        ...
        if (ordertypeID == 3) {
            $.ajax({
                ...
                success: function(result) {
                    ...
                    $.each(obj, function(i, item) {
                        ...
                        html1 += obj[i].name + ' - ' + obj[i].materialname + ' / ' + obj[i].materialinfocode;
                        html1 += '</option>';
                    });
            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    html1 += '<option value="">Select</option>';
                    $.each(obj, function(i, item) {
                        html1 += '<option value="' + obj[i].idtbl_spareparts + '">';
        ...
        if (!$("#createorderform")[0].checkValidity()) {
            ...
            if (ordertype == 1 || ordertype == 3 || ordertype == 4) {
                ...
                $('#tableorder > tbody:last').append('<tr class="pointer"><td>' + product +
                    ...
                    uom + '</td> <td class="text-right d-none">' +
                    unitprice + '</td><td class="d-none">' + uomID +
                    '</td><td class="d-none">' + peices +
