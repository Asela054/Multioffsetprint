...
<script>
	$(document).ready(function () {
		...
		$('#tblmaterialissue').DataTable({
			...
			ajax: {
				url: "<?php echo base_url() ?>scripts/issuemateriallist.php",
			...
			"columns": [{
					"data": "idtbl_customerinquiry_detail"
				},
				{
					"data": "name"
				},
...

				{
					...
					"render": function (data, type, full) {
						...
						button += '" id="' + full['idtbl_customerinquiry_detail'] +
    					'" data-approvestatus="' + full['is_materialissue_approved'] + '"><i class="fas fa-eye"></i></button>';
						if (full['is_materialissue_approved'] == 0) {
							button +=
								'<a href="<?php echo base_url() ?>IssueMaterials/ApproveMaterialIssue/' +
								full['idtbl_customerinquiry_detail'] +
								'" onclick="return issue_confirm()" target="_self" class="btn btn-secondary btn-sm mr-1 ';
							if (statuscheck != 1) {
								button += 'd-none';
							}
				...
				success: function (result) {
					...
					$.each(obj, function (i, item) {
						...
						$('#tblmaterialdetails> tbody:last').append(
							'<tr><td class="text-center">' + obj[i].job +
							'</td><td class="text-center">' + obj[i].materialname +
