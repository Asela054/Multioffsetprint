											...
											<form id="createorderform" autocomplete="off">
												...
												<div class="form-group mb-1">
													<label class="small font-weight-bold text-dark">Customer
														Inquiry*</label>
													<select class="form-control form-control-sm" name="customerinqury"
														id="customerinqury">
														<option value="">Select</option>
														<?php foreach($grnlist->result() as $rowgrnlist){ ?>
														<option
															value="<?php echo $rowgrnlist->idtbl_customerinquiry ?>">
															<?php echo 'CUI000'.$rowgrnlist->idtbl_customerinquiry ?>
														</option>
														<?php } ?>
													</select>
												</div>
												...
												<div class="form-group mb-1">
													...
													<input type="number" id="ponum" name="ponum"
...
<script>
			...
			"columns": [
				...
				// },
				{
					...
					"render": function (data, type, full) {
						...
						// button += '<a href="<?php echo base_url() ?>Issuegoodreceive/Issuepdf/' +
						// 	full['idtbl_print_issue'] +
						// 	'" target="_blank" class="btn btn-secondary btn-sm btnPdf mr-1" data-toggle="tooltip" data-placement="bottom" title="Issue Item Request PDF"><i class="fas fa-file-pdf"></i></a>';
		...
		$("#formsubmit").click(function () {
			if (!$("#createorderform")[0].checkValidity()) {
				...
				$('#tableorder > tbody:last').append('<tr class="pointer"><td name="job">' +
					job +'</td><td name="comment" class="text-center">' +
					comment + '</td><td name="qty" class="text-center">' + newqty +
					'</td><td name="productid" class="d-none">' + jobID +
		...
		$('#btncreateorder').click(function () { //alert('IN');
			...
			// if (ordertype == 3 || ordertype == 4) {
				...
				if (tbody.children().length > 0) {
					...
					//alert("click");
					var date = $('#date').val();
					var customerinqury = $('#customerinqury').val();
					var ponum = $('#ponum').val();
	...
	$('#job').change(function () {
		var jobID = $(this).val();
		var inqury_id = $('#customerinqury').val();
		console.log(jobID);
		//alert(jobID);
