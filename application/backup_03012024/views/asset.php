                            ...
                            <div class="col-3">
                                <form action="<?php echo base_url() ?>Asset/Assetinsertupdate" method="post" autocomplete="off">
								    <div class="form-group mb-1">
                                        <label class="small font-weight-bold">Asset Sub Category*</label>
                                        <select class="form-control form-control-sm" name="Asset_cat" id="Asset_cat" required>
                                            <option value="">Select</option>
                                            <?php foreach ($Assetsubtypelist->result() as $rowAssettypelist) { ?>
                                            <option value="<?php echo $rowAssettypelist->idtbl_assetsub_category ?>"><?php echo $rowAssettypelist->sub_category ?></option>
                                            <?php } ?>
                                        </select>
                                    </div>
								    <div class="form-group mb-1">
                                        <label class="small font-weight-bold">Purchase Date*</label>
                                        <input type="date" class="form-control form-control-sm" name="purdate" id="purdate" required>
                                    </div>
									<div class="form-group mb-1">
                                        <label class="small font-weight-bold">Purchase Price*</label>
                                        <input type="number" step="any" class="form-control form-control-sm" name="purprice" id="purprice" required>
                                    </div>
									<div class="form-group mb-1">
                                        <label class="small font-weight-bold"> Date Started to use*</label>
                                        <input type="date" class="form-control form-control-sm" name="usedate" id="usedate" >
                                    </div>
                                    <div class="form-group mb-1">
                                    ...
                                    <div class="form-group mb-1">
                                        <label class="small font-weight-bold"> Discreption*</label>
                                        <textarea name="discription" class="form-control form-control-sm" id="discription" cols="10" rows="4"required></textarea>
                                    </div>
            ...
            if (r == true) {
                ...
                $.ajax({
                    ...
                    success: function(result) { //alert(result);
                        ...
                        $('#recordID').val(obj.id);
                        $('#purdate').val(obj.Purdate); 
						$('#purprice').val(obj.purprice); 
						$('#usedate').val(obj.date); 
						$('#location').val(obj.location); 
                        $('#code').val(obj.code); 
                        $('#discription').val(obj.discription); 
                        $('#Asset_cat').val(obj.idtbl_assetsub_category); 
