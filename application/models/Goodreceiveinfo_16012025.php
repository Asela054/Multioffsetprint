<?php class Goodreceiveinfo extends CI_Model {
	...
	public function Goodreceiveinsertupdate() {
		...
		$query = $this->db->query("SELECT grn_no FROM tbl_print_grn WHERE company_id = $comapnyID AND `grndate` BETWEEN '$fromyear' AND '$toyear' ORDER BY grn_no DESC LIMIT 1");
		
		if ($query->num_rows() > 0) {
			$last_grn_no = $query->row()->grn_no;
		...
		$grnID=$this->db->insert_id();
			...
			$grn_no = 'GRN' . $yearDigit . $countPrefix;
			...
			$this->db->update('tbl_print_grn', $invodetail);
	...
	public function Goodreceiveview() {
		...
		$sql="SELECT `u`.*, `ua`.`suppliername`, `ua`.`telephone_no`, `ua`.`address_line1`, `ub`.`location`, `ub`.`phone`, `ub`.`address`, `ub`.`phone2`, `ub`.`email` AS `locemail` FROM `tbl_print_grn` AS `u` LEFT JOIN `tbl_supplier` AS `ua` ON (`ua`.`idtbl_supplier` = `u`.`tbl_supplier_idtbl_supplier`) LEFT JOIN `tbl_location` AS `ub` ON (`ub`.`idtbl_location` = `u`.`tbl_location_idtbl_location`) WHERE `u`.`status`=? AND `u`.`idtbl_print_grn`=?";
		$respond=$this->db->query($sql, array(1, $recordID));
	...
	public function Goodreceivestatus($x, $y, $z) {
		...
		if($type==1) {
			...
			$this->db->select('tbl_print_grn.batchno, tbl_print_grn.company_id, tbl_print_grn.company_branch_id, tbl_print_grn.grntype, tbl_print_grn.tbl_location_idtbl_location, tbl_print_grn.tbl_supplier_idtbl_supplier, tbl_print_grn.grndate, tbl_print_grndetail.qty, tbl_print_grndetail.pieces, tbl_print_grndetail.total, tbl_print_grndetail.measure_type_id, tbl_print_grndetail.unitprice, tbl_print_grndetail.tbl_print_material_info_idtbl_print_material_info, tbl_print_grndetail.tbl_machine_id, tbl_print_grndetail.tbl_sparepart_id');
			$this->db->from('tbl_print_grn');
			...
			if ($respond->num_rows() > 0) {
				foreach ($respond->result() as $row) {
					...
					$branchid = $row->company_branch_id;
	...
	public function Getproductinfoaccoproduct() {
		...
		$this->db->select('`qty`, `unitprice`, `pieces`, `comment`, `measure_type_id`,`idtbl_print_porder_detail`,`actual_qty`');
		$this->db->from('tbl_print_porder_detail');
		$this->db->where('status', 1);
	...
	public function Getproductinfoamachine() {
		...
		$this->db->select('`qty`, `unitprice`, `comment` ,`measure_type_id`,`idtbl_print_porder_detail`,`actual_qty`');
		$this->db->from('tbl_print_porder_detail');
		$this->db->where('status', 1);
