...
class Invoiceinfo extends CI_Model{
    ...
    public function Getqtyaccdispatch() {
        ...
        if($respond->num_rows()>0) {
            $obj=new stdClass();
            $dispatchQty = $respond->row(0)->qty;

            // Get sum of credit note qtys for this dispatch
            $this->db->select_sum('cnd.qty');
            $this->db->from('tbl_credit_note_detail cnd');
            $this->db->join('tbl_credit_note cn', 'cn.idtbl_credit_note = cnd.tbl_credit_note_idtbl_credit_note');
            $this->db->where('cnd.dispatch_id', $recordID);
            $this->db->where('cn.approvestatus', 1);
            $this->db->where('cn.status', 1);
            $creditQuery = $this->db->get();
            $creditQty = $creditQuery->row()->qty ? $creditQuery->row()->qty : 0;

            $availableQty = $dispatchQty - $creditQty;
            if ($availableQty < 0) {
                $availableQty = 0;
            }

            $obj->qty = $availableQty;
