<?php class Goodreceiveinfo extends CI_Model {
	...
	public function Goodreceiveview() {
		...
		$this->db->select('tbl_print_grndetail.*,tbl_print_grn.grndate,tbl_print_grn.grn_no,tbl_print_grn.tbl_order_type_idtbl_order_type, tbl_print_material_info.materialinfocode, tbl_print_material_info.materialname,tbl_machine.machine,tbl_machine.machinecode,tbl_spareparts.spare_part_name,tbl_measurements.measure_type');
	...
	public function Approvestatus() {
		...
		if ($confirmnot == 1) {
			$this->db->select('qty, pieces, tbl_print_material_info_idtbl_print_material_info, tbl_machine_id, tbl_sparepart_id');
			$this->db->from('tbl_print_grndetail');
			...
			foreach ($grnDetails->result() as $detail) {
				...
				} elseif (!empty($detail->tbl_machine_id)) {
					$this->db->where('tbl_machine_id', $detail->tbl_machine_id);
				} elseif (!empty($detail->tbl_sparepart_id)) {
					$this->db->where('tbl_sparepart_id', $detail->tbl_sparepart_id);
				}
			...
			$this->db->select('SUM(qty) as total_qty, SUM(actual_qty) as total_actual_qty');
			$this->db->where('tbl_print_porder_idtbl_print_porder', $porderID);
			$qty_check = $this->db->get('tbl_print_porder_detail')->row();

			if ($qty_check->total_actual_qty >= $qty_check->total_qty) {
				$dataporder = array(
					'grnconfirm' => '1', 
					'updateuser' => $userID,
					'updatedatetime' => $updatedatetime
		...
		if ($this->db->trans_status() === TRUE) {
			...
			if ($confirmnot == 1) {
				...
				if (isset($qty_check) && $qty_check->total_actual_qty < $qty_check->total_qty) {
					$actionObj->message .= ' (GRN not fully confirmed - quantities not met)';
				}
			} else {
			...
			if ($confirmnot == 1 && isset($qty_check)) {
				$response['grnconfirmed'] = ($qty_check->total_actual_qty >= $qty_check->total_qty) ? 1 : 0;
			}
