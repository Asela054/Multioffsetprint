...
class PdfviewDirectInvoiceinfo extends CI_Model {
    ...
    public function pdfdata($x) {
		...
		$this->db->select('*, COALESCE(tbl_direct_invoice.idtbl_direct_invoice, 0) AS idtbl_direct_invoice, COALESCE(tbl_direct_invoice.vat, 0) AS vatpercent, COALESCE(tbl_direct_invoice.vat_amount, 0) AS vatamount, COALESCE(tbl_direct_invoice.inv_no, 0) AS inv_number, COALESCE(tbl_direct_invoice.date, 0) AS invoice_date, COALESCE(tbl_direct_invoicedetail.total, 0) AS detail_total, COALESCE(tbl_direct_invoice.total, 0) AS net_total, COALESCE(tbl_direct_invoicedetail.unitprice, 0) AS unit_price, COALESCE(tbl_direct_invoicedetail.qty, 0) AS quantity,COALESCE(tbl_print_material_info.materialname, 0) As materialname');
        $this->db->from('tbl_direct_invoice');
		$this->db->join('tbl_direct_invoicedetail', 'tbl_direct_invoice.idtbl_direct_invoice = tbl_direct_invoicedetail.tbl_direct_invoice_idtbl_direct_invoice', 'left');
        ...
        $this->db->join('tbl_print_material_info', 'tbl_print_material_info.idtbl_print_material_info = tbl_direct_invoicedetail.tbl_print_material_info_idtbl_print_material_info');
        $this->db->join('tbl_measurements', 'tbl_direct_invoicedetail.tbl_measurements_idtbl_mesurements = tbl_measurements.idtbl_mesurements', 'left');
		...
		$this->db->select('tbl_direct_invoice_othercharges.charge_amount, tbl_charges.charges_type, tbl_direct_invoice.vat, tbl_customer.vat_customer');
		$this->db->from('tbl_direct_invoice');
                 ...
                 <tr>
                    <td colspan="3">
                        <table class="tablec" border="0">
                            <thead class="thc">
                                <tr>
								    <th class="thc" width="15%" style="border: 1px solid black; border-left: 0px solid black; border-right: 0px solid black;"><strong>Material</strong></th>
                                    <th width="15%" style="border: 1px solid black; border-left: 0px solid black; border-right: 0px solid black;"></th>
                                    ...
                                    <th class="thc" style="text-align:center; border: 1px solid black; border-left: 0px solid black; border-right: 0px solid black;" width="10%"><strong>UOM</strong></th>
                                    <th class="thc" width="25%" style="text-align:right; border: 1px solid black; border-left: 0px solid black; border-right: 0px solid black;"><strong>Unit Price</strong></th>
							...
							foreach ($query->result() as $row) {
								...
								$html .= '
									<tr>
										<td class="tdc" style="width: 350px;">' . $row->materialname .'</td>
                                        <td></td>
