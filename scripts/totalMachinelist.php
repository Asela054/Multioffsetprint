...
$joinQuery = "FROM `tbl_print_stock` AS `u` LEFT JOIN `tbl_machine` AS `ua` ON (`ua`.`idtbl_machine` = `u`.`tbl_machine_id`)
			LEFT JOIN `tbl_supplier` AS `us` ON (`us`.`idtbl_supplier` = `u`.`supplier_id`)";

$extraWhere = "`u`.`tbl_machine_id` <> 0 AND `u`.`status` IN (1, 2) AND `u`.`tbl_company_idtbl_company`='$companyid'";
