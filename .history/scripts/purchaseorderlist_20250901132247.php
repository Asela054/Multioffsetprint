...
$joinQuery = "FROM `tbl_print_porder` AS `u` LEFT JOIN `tbl_supplier` AS `ua` ON (`ua`.`idtbl_supplier` = `u`.`tbl_supplier_idtbl_supplier`) LEFT JOIN `tbl_material_` AS `uc` ON (`uc`.`idtbl_material_` = `u`.`tbl_material__idtbl_material_`) LEFT JOIN `tbl_print_porder_req` AS `ud` ON (`ud`.`idtbl_print_porder_req` = `u`.`tbl_print_porder_req_idtbl_print_porder_req`) LEFT JOIN `tbl_user` AS `ue` ON (`ue`.`idtbl_user` = `u`.`check_by`)";
