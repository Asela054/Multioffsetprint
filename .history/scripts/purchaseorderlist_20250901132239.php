...
$joinQuery = "FROM `tbl_print_porder` AS `u` LEFT JOIN `tbl_supplier` AS `ua` ON (`ua`.`idtbl_supplier` = `u`.`tbl_supplier_idtbl_supplier`) LEFT JOIN `tbl` AS `uc` ON (`uc`.`idtbl` = `u`.`tbl_idtbl`) LEFT JOIN `tbl_print_porder_req` AS `ud` ON (`ud`.`idtbl_print_porder_req` = `u`.`tbl_print_porder_req_idtbl_print_porder_req`) LEFT JOIN `tbl_user` AS `ue` ON (`ue`.`idtbl_user` = `u`.`check_by`)";
