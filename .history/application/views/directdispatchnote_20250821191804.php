								...
								<form id="createorderform" autocomplete="off">
									<div class="form-row mb-1">
										...
										<div class="col">
											...
											<select id="customer" name="customer" class="form-control form-control-sm" required>
                                    ...
                                    <div class="form-row mb-1">
                                        <div class="col">
                                            ...
                                            <select name="material" id="material" class="form-control form-control-sm" style="width: 100%;" required>
                                                <option value="">Select</option>
                                            </select>
                                        </div>
										<div class="col">
                                            <lable class="font-weight-bold small">Batch No* </lable>
                                            <select name="batchno" id="batchno" class="form-control form-control-sm" style="width: 100%;" required>
                                                <option value="">Select</option>
                                            </select>
                                        </div>
                                    </div>
...
$(document).ready(function () {
    ...
    $("#material").select2({
        ajax: {
            url: "<?php echo base_url() ?>DirectDispatchnote/GetAllMaterialList",
            type: "post",
            dataType: 'json',
            delay: 250,
            data: function (params) {
	...
	$('#material').change(function () {
		...
		$.ajax({
			...
			url: 'DirectDispatchnote/Getbatchlist',
			success: function (result) {
				$('#batchno').empty();
				$('#batchno').append('<option value="">Select Batch</option>');

				if (result && result.length > 0) {
					try {
						var batches = JSON.parse(result);
						$.each(batches, function (index, batch) {
							$('#batchno').append('<option value="' + batch.id + '">' + batch.batchno + ' - ' + batch.qty +' </option>');
						});
					} catch (e) {
	...
	$('#dataTable').DataTable({
		...
		ajax: {
			url: "<?php echo base_url() ?>scripts/directdispatchlist.php",
		...
		"columns": [{
			...
			},
			{
				...
				"render": function (data, type, full) {
					var button = '';
					button += '<a href="<?php echo base_url() ?>DirectDispatchnote/pdfget/' + full[
							'idtbl_direct_dispatch'] +
					...
					if (full['status'] == 1) {
						button += '<button data-toggle="tooltip" data-placement="bottom" title="View Dispatch" class="btn btn-dark btn-sm btnview mr-1" id="' +
							full[
								'idtbl_direct_dispatch'] + '"  aproval_id="' + full[
								'approvestatus'] + '" check_status="' + full[
                            'check_by'] + '"><i class="fas fa-eye"></i></button>';
					}
					if (full['approvestatus'] == 0) {
						button +=
							'<a href="<?php echo base_url() ?>DirectDispatchnote/DirectDispatchnotestatus/' +
							full['idtbl_direct_dispatch'] +
							'/3/' +
	...
	$('#dataTable tbody').on('click', '.btnview', function () {
		...
		$.ajax({
			...
			url: '<?php echo base_url() ?>DirectDispatchnote/DirectDispatchview',
			success: function (result) {
		...
		$.ajax({
			...
			url: '<?php echo base_url() ?>DirectDispatchnote/Directdispatchviewheader',
	...
	$('#btncheck').click(function () {
		Swal.fire({
			title: "Do you want to check this Dispatch Note?",
			showDenyButton: true,
			showCancelButton: true,
	...
	$("#formsubmit").click(function () {
		if (!$("#createorderform")[0].checkValidity()) {
			...
			$("#submitBtn").click();
				...
				var fullText = $("#batchno option:selected").text();
				var batchno = fullText.split(" - ")[0];

				$('#tableorder > tbody:last').append('<tr class="pointer"><td name="material">' +
					material + '</td><td name="materialid" class="d-none">' + materialID +
					'</td><td name="comment" class="text-center">' +
					comment + '</td><td name="qty" class="text-center">' + newqty +
					'</td><td name="uom"class="text-center">' + uom +
					'</td><td name="uomID" class="d-none">' + uomID +
					'</td><td name="uomID" class="d-none">' + batchno +
			...
			Swal.fire({
				...
				didOpen: () => {
					...
					$.ajax({
						...
						url: 'DirectDispatchnote/DirectDispatchnoteinsertupdate',
						success: function (result) {
							Swal.close();
    ...
    Swal.fire({
        ...
        didOpen: () => {
            ...
            $.ajax({
                ...
                url: '<?php echo base_url() ?>DirectDispatchnote/Approdispatch',
                success: function(result) {
                    Swal.close();
                    document.body.style.overflow = 'auto';
                    var obj = JSON.parse(result);
    ...
    Swal.fire({
        ...
        didOpen: () => {
            ...
            $.ajax({
                type: "POST",
                data: {
                    requestid: $('#dispatchid').val(),
                    confirmnot: confirmnot
                },
                url: '<?php echo base_url() ?>DirectDispatchnote/Dispatchnotecheckstatus',
                success: function(result) {
                    Swal.close();
                    document.body.style.overflow = 'auto';
                    var obj = JSON.parse(result);
