...
$(document).ready(function() {
    ...
    $("#editproduct").select2({
		...
		width: '100%',
		ajax: {
			url: "<?php echo base_url() ?>Service/GetProductList",
			type: "post",
			dataType: 'json',
			delay: 250,
    	...
    	Swal.fire({
    		...
    		if (result.isConfirmed) {
    			...
    			$.ajax({
    				...
    				url: '<?php echo base_url() ?>Service/Purchaseorderedit',
    ...
    $('#editbtncreateorder').click(function () {
        ...
        $.ajax({
            type: "POST",
            url: "Service/Purchaseorderupdate",
            data: orderData,
            success: function (result) {
    ...
    $('#editproduct').change(function () {
    	...
    	var supplier = $('#editsupplier').val();
    		...
    		$.ajax({
    			type: "POST",
    			url: 'Service/Getproductinfoaccoproduct',
    			data: {
    				recordID: productID,
    				supplier: supplier 			
                },
