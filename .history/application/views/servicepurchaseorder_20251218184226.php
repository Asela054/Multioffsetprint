...
$(document).ready(function() {
    ...
    var materialGroup = item.materialgroup;

    var product = item.material;   // define first
    var comment = item.comment;    // define first

    // ✅ now safe to use
    var displayName = (materialGroup == 4) ? comment : product;
    ...
    $('#edittableorder > tbody:last').append(
        '<tr class="pointer">' +
            ...
            '<td class="edittotal d-none">' + netprice + '</td>' +
            '<td class="text-right">' + showtotal + '</td>' +
