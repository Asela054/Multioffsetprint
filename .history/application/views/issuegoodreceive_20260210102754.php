...
function approvejob(confirmnot) {
    Swal.fire({
        ...
        didOpen: () => {
            ...
            // Collect table data (example: rows of stock id and qty)
            var tableData = [];
            $('#issueTable tbody tr').each(function() {
                var stockid = $(this).find('.stockid').text(); // adjust selector to your table
                var qty = $(this).find('.issueqty').text();     // adjust selector
                if (stockid && qty) {
                    tableData.push({ stockid: stockid, qty: parseFloat(qty) });
                }
            });
            ...
            $.ajax({
                ...
                data: {
                    ...
                    tableData: JSON.stringify(tableData) // send as JSON
                },
                success: function(result) {
                    Swal.close();
                    document.body.style.overflow = 'auto';
