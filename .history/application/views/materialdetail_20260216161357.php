            ...
            $.ajax({
                ...
                success: function(result) { //alert(result);
                    ...
                    $('#materialgroup').val(obj.materialgroup);
                     if (obj.customerid && obj.customername) {
        var newOption = new Option(obj.customername, obj.customerid, true, true);
        $('#customer').append(newOption).trigger('change');
    } 
    // If customer is just an ID and you need to fetch the text
    else if (obj.customer) {
        // You might need to make an additional AJAX call to get the customer name
        $.ajax({
            url: "<?php echo base_url() ?>Materialdetail/Getcustomerbyid",
            type: "post",
            data: {id: obj.customer},
            success: function(customerData) {
                var customer = JSON.parse(customerData);
                var newOption = new Option(customer.name, customer.id, true, true);
                $('#customer').append(newOption).trigger('change');
            }
        });
    }
