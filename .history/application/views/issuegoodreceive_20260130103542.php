	...
	$(document).ready(function () {
		...
		$('#approvel').click(async function () {
			...
			$("#approveltable tbody tr").each(function () {
				...
				item["qty"] = $(this).find('td:eq(1)').text();
				item["materialid"] = $(this).find('td:eq(2)').text();
				item["stockid"] = $(this).find('td:eq(3)').text();
		...
		$("#formsubmit").click(function () {
			if (!$("#createorderform")[0].checkValidity()) {
				...
				$('#tableorder > tbody:last').append('<tr class="pointer"><td name="productname">' +
					product +
					'</td><td>' + uom +
						'</td><td class="">' + uomID + '</td><td name="comment">' +
		...
		var itemreq_id = $('#itemrequest').val();
			...
			$.ajax({
				...
				success: function (result) {
					...
					$.each(obj, function (i, item) {
						html1 += '<option value="' + item.batchno + '">' + item.batchno + ' / Stock in Hand - ' + item.qty + ' </option>';
					});
