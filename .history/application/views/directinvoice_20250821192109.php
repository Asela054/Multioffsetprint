            					...
            					<form id="createorderform" autocomplete="off">
            						<div class="row">
            							<div class="col-3">
            								...
            								<input type="date" class="form-control form-control-sm" placeholder="" name="date"
            									id="date" onchange="getVat();" value="<?php echo date('Y-m-d') ?>" required>
            							</div>
            							<div class="col-3">
            								<label class="small font-weight-bold text-dark">Dispatch Note*</label>
            								<select class="form-control form-control-sm  px-0" name="dispatch_note" id="dispatch_note"
            									required>
            									<option value="">Select</option>
                                                <?php foreach($dispatchlist->result() as $rowdispatchlist){ ?>
            									<option value="<?php echo $rowdispatchlist->idtbl_direct_dispatch ?>">
            										<?php echo $rowdispatchlist->dispatch_no ?></option>
            									<?php } ?>
            								</select>
            							</div>
                                        <div class="col-3">
            								<label class="small font-weight-bold text-dark">Material*</label>
            								<select class="form-control form-control-sm  px-0" name="material" id="material"
            									required>
            									<option value="">Select</option>
            								</select>
            							</div>
            						...
            						<div class="row">
            							<div class="col-3">
            								...
            								<select class="form-control form-control-sm" name="uom" id="uom" required readonly>
            								</select>
            							</div>
            							<div class="col-3">
                        ...
                        <div class="row">
                            ...
                            <div class="col-1">
                                ...
                                <input type="text" id="vat" name="vat" class="form-control form-control-sm"
                                    value="<?php echo $vatValue; ?>" onkeyup="finaltotalcalculate();" required readonly>
                            </div>
...
$(document).ready(function() {
    ...
    $('#dispatch_note').change(function () {
        ...
        $.ajax({
            ...
            url: 'DirectInvoice/Getmateriallist',
            success: function (result) {
                $('#material').empty();
                $('#material').append('<option value="">Select Material</option>');

                if (result && result.length > 0) {
                    try {
                        var materials = JSON.parse(result);
                        $.each(materials, function (index, material) {
                            $('#material').append('<option value="' + material.id + '">' + material.material + ' / ' + material.batchno + ' </option>');
                        });
                    } catch (e) {
    ...
    $('#material').change(function () {
        ...
        var batchno = '';
        if (selectedOptionText.includes('/')) {
            batchno = selectedOptionText.split('/')[1].trim();
        }
        ...
        $.ajax({
            ...
            url: 'DirectInvoice/Getunitprice',
            success: function (result) {
                $('#unitprice').val('');
                
                if (result && result.length > 0) {

                $('#unitprice').val(result[0].saleprice);
            }
            },
            error: function (xhr, status, error) {
                console.error("AJAX error:", error);
    ...
    $('#material').change(function () {
        var materialID = $(this).val();
        var dispatchID = $('#dispatch_note').val();
        ...
        $.ajax({
            ...
            url: 'DirectInvoice/Getdispatchdetails',
            success: function (result) {
                $('#uom').empty();
                $('#newqty').val('');
                ...
                if (result && result.length > 0) {
                    try {
                        var dispatchdetails = JSON.parse(result);

                        $.each(dispatchdetails, function (index, dispatch) {
                            $('#uom').append('<option value="' + dispatch.id + '">' + dispatch.type + ' </option>');
                        });

                        $('#newqty').val(dispatchdetails[0].material);

                    } catch (e) {
                        console.error("Error parsing uom:", e);
                    }
                }
            },
            error: function (xhr, status, error) {
    ...
    $('#dataTable').DataTable({
        ...
        ajax: {
            url: "<?php echo base_url() ?>scripts/directinvoicelist.php",
        ...
        "columns": [
            ...
            },
            {
                ...
                "render": function(data, type, full) {
                    ...
                    var button = '';
                    button += '<a href="<?php echo base_url() ?>DirectInvoice/pdfget/' + full[
                            'idtbl_direct_invoice'] +
                    ...
                    button += '<button data-toggle="tooltip" data-placement="bottom" title="View Invoice" class="btn btn-dark btn-sm btnview mr-1" id="' +
                        full[
                            'idtbl_direct_invoice'] + '" aproval_id="' + full[
                            'approvestatus'] + '" check_status="' + full[
                            'check_by'] + '" request_id="' + full[
                            'tbl_direct_dispatch_idtbl_direct_dispatch'] +
                    ...
                    if (full['approvestatus'] == 0) {
                        button +=
                            '<a href="<?php echo base_url() ?>DirectInvoice/DirectInvoicestatus/' +
                            full['idtbl_direct_invoice'] +
    ...
    $('#dataTable tbody').on('click', '.btnview', function() {
        ...
        $.ajax({
            ...
            url: '<?php echo base_url() ?>DirectInvoice/DirectInvoiceview',
            success: function(result) {
                console.log(result);
        ...
        $.ajax({
            ...
            url: '<?php echo base_url() ?>DirectInvoice/DirectInvoiceviewheader',
    ...
    $('#btncheck').click(function () {
		Swal.fire({
			title: "Do you want to check this Invoice?",
			showDenyButton: true,
			showCancelButton: true,
    ...
    $("#formsubmit").click(function() {
        if (!$("#createorderform")[0].checkValidity()) {
            ...
            var fullText = $("#material option:selected").text();
			var material = fullText.split(" - ")[0];


                $('#tableorder > tbody:last').append('<tr class="pointer"><td name="material" class="text-left">' + material +
                    '</td><td name="materialID" class="d-none">' + materialID +
                    '</td><td name="qty" class="text-center">' + newqty +
    ...
    $('#btncreateorder').click(function () {
    	...
    	if ($('#tableorder > tbody').is(':empty')) {
    		...
    		var date = $('#date').val();
    		var dispatch_note = $('#dispatch_note').val();
    		...
    		Swal.fire({
    			...
    			didOpen: () => {
    				...
    				$.ajax({
    					...
    					url: 'DirectInvoice/DirectInvoiceinsertupdate',
    					success: function (result) {
    						Swal.close();
    ...
    Swal.fire({
        ...
        didOpen: () => {
            ...
            $.ajax({
                ...
                url: '<?php echo base_url() ?>DirectInvoice/Approdirectinvoice',
                success: function(result) {
                    Swal.close();
                    document.body.style.overflow = 'auto';
                    var obj = JSON.parse(result);
    ...
    Swal.fire({
        ...
        didOpen: () => {
            ...
            $.ajax({
                type: "POST",
                data: {
                    requestid: $('#invoiceid').val(),
                    confirmnot: confirmnot
                },
                url: '<?php echo base_url() ?>DirectInvoice/DirectInvoicecheckstatus',
                success: function(result) {
                    Swal.close();
                    document.body.style.overflow = 'auto';
                    var obj = JSON.parse(result);
