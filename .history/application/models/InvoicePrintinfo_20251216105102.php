...
<td width="55%" style="vertical-align: top;padding:0px;">
    <p style="margin:0;font-size:16px;font-weight:bold;">PURCHASE ORDER</p>
    <p style="margin:0;font-size:13px;font-weight:bold;">
        To: <?= $respond->row(0)->suppliername ?>
    </p>

    <?php if (!empty($respond->row(0)->address_line1)) { ?>
        <p style="margin:0;font-size:13px;padding-left:24px;">
            <?= $respond->row(0)->address_line1 ?>,
        </p>
    <?php } ?>

    <?php if (!empty($respond->row(0)->address_line2)) { ?>
        <p style="margin:0;font-size:13px;padding-left:24px;">
            <?= $respond->row(0)->address_line2 ?>,
        </p>
    <?php } ?>

    <?php if (!empty($respond->row(0)->city)) { ?>
        <p style="margin:0;font-size:13px;padding-left:24px;">
            <?= $respond->row(0)->city ?>.
        </p>
    <?php } ?>

    <?php if (!empty($tpnumber)) { ?>
        <p style="margin:0;font-size:13px;padding-left:24px;">
            <?= $tpnumber ?>
        </p>
    <?php } ?>
