...
class PdfGRNinfo extends CI_Model {
    public function pdfgrnget($x) {
        ...
        $this->db->select('*, COALESCE(tbl_print_grn.idtbl_print_grn, 0) AS idtbl_print_grn, COALESCE(tbl_print_grn.totalcost, 0) AS grn_total, COALESCE(tbl_print_grn.discount, 0) AS discount, COALESCE(tbl_print_grndetail.qty, 0) AS qty, COALESCE(tbl_print_grndetail.costunitprice, 0) AS costunitprice, COALESCE(tbl_material_group.idtbl_material_group, 0) AS idtbl_material_group , COALESCE(tbl_print_grndetail.comment, 0) AS comment');
