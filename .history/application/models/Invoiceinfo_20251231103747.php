...
public function Approinvoice()
{
    ...
    $recordID   = (int) $this->input->post('invoiceid');
    $disid      = (int) $this->input->post('reqestid');
    $jobid      = (int) $this->input->post('hiddenjobid');
    $confirmnot = (int) $this->input->post('confirmnot');
    $finishJob  = (int) $this->input->post('finish_job');
    ...
    try {
        ...
        // 🔹 Finish job ONLY if approved + checkbox checked
        if ($confirmnot === 1 && $finishJob === 1 && $jobid > 0) {
            ...
            if ($this->db->affected_rows() == 0) {
                throw new Exception('Job finish update failed.');
            }
        }

        // 🔹 Update dispatch invoice status (approve only)
        if ($confirmnot === 1) {

            $dispatchDetails = $this->db
                ->select('tbl_print_dispatch_idtbl_print_dispatch')
                ->from('tbl_print_invoicedetail')
                ->where('tbl_print_invoice_idtbl_print_invoice', $recordID)
                ->where('status', 1)
                ->get()
                ->result();
            ...
            foreach ($dispatchDetails as $row) {
                if (!empty($row->tbl_print_dispatch_idtbl_print_dispatch)) {
                    $this->db->where('idtbl_print_dispatch', $row->tbl_print_dispatch_idtbl_print_dispatch);
                    $this->db->update('tbl_print_dispatch', [
                        'invoice_status' => 1,
                        'updateuser'     => $userID,
                        'updatedatetime' => $updatedatetime
                    ]);
        ...
        $actionObj->title = '';
        $actionObj->message = ($confirmnot === 1)
