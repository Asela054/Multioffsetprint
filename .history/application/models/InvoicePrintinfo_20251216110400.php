...
// Build address array with proper punctuation
$addressParts = [];
if (!empty($respond->row(0)->address_line1)) {
    $addressParts[] = $respond->row(0)->address_line1 . ',';
}
if (!empty($respond->row(0)->address_line2)) {
    $addressParts[] = $respond->row(0)->address_line2 . ',';
}
if (!empty($respond->row(0)->city)) {
    $addressParts[] = $respond->row(0)->city . '.';
}

// Display address lines
foreach ($addressParts as $addressPart) {
    $html .= '<p style="margin:0px;font-size:13px;padding-left: 24px;">' . $addressPart . '</p>';
}

// Display telephone if exists
if ($tpnumber != '&nbsp;') {
