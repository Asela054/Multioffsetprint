	...
	public function Approveissue()
	{
		...
		if (!empty($tableData) && is_array($tableData)) {
			...
			foreach ($tableData as $rowtabledata) {
				...
				$stockid      = (int) $rowtabledata['stockid'];
				$issueqty     = (float) $rowtabledata['qty'];
				...
				if (!empty($account_id) && is_numeric($account_type)) {
					if ($account_type == 1) {
				...
				Update Issue Detail
				========================== */
				$issueDetailData = array_merge(array(
					'updateuser' => $userID,
					'updatedatetime' => $updatedatetime
				), $accountData);
				...
				$this->db->update('tbl_print_issuedetail', $issueDetailData);
			}
		}
