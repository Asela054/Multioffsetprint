...
public function stockReport() {
    ...
    // 1️⃣ Filter by material group if a normal category is selected
    if ($category != '' && $category != '0' && $category != 'by_customer') {
        $this->db->where('tbl_print_material_info.tbl_material_group_idtbl_material_group', $category);
    }

    // 2️⃣ If "By Customer" selected, filter by customer if selected
    if ($category == 'by_customer' && !empty($customer)) {
        $this->db->where('tbl_print_material_info.tbl_customer_idtbl_customer', $customer);
    }

    // 3️⃣ Optional: if "By Customer" selected but customer is empty → show all materials
    // Uncomment if you want this behavior
    // if ($category == 'by_customer' && empty($customer)) {
    //     $this->db->where('tbl_print_material_info.tbl_customer_idtbl_customer IS NOT NULL', null, false);
    // }
