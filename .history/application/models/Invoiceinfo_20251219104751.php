...
public function Approinvoice()
{
    ...
    $requestID = $this->input->post('reqestid');
    ...
    try {
        ...
        // 2️⃣ If confirmed, process dispatch notes ONE BY ONE
        if ($confirmnot == 1) {

            // Get all dispatch notes for this request
            $dispatches = $this->db
            ...
            foreach ($dispatches as $dispatch) {
                ...
                if ($dispatch->approvestatus != 1) {
                    throw new Exception('Dispatch not approved: ' . $dispatch->dispatch_no);
                }

                // Update ONE dispatch at a time
