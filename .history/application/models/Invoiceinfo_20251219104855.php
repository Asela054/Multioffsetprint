...
public function Approinvoice()
{
    ...
    $requestID = $this->input->post('reqestid'); // inquiry id
    ...
    try {
        ...
        if ($confirmnot == 1) {
            ...
            /* -------- GET ALL DISPATCH NOTES (BULK) -------- */
            $dispatches = $this->db
                ->where('tbl_customerinquiry_idtbl_customerinquiry', $requestID)
                ->where('status', 1)
                ->get('tbl_print_dispatch')
                ->result();

            if (empty($dispatches)) {
                throw new Exception('No dispatch notes found for this inquiry');
            }

            /* -------- LOOP & UPDATE ONE BY ONE -------- */
            foreach ($dispatches as $dispatch) {

                // Optional check before update
                if ($dispatch->invoice_status == 1) {
                    continue; // already invoiced
                }

                $this->db->where('idtbl_print_dispatch', $dispatch->idtbl_print_dispatch);
                ...
                if ($this->db->affected_rows() == 0) {
                    throw new Exception(
                        'Failed to update dispatch ID ' . $dispatch->idtbl_print_dispatch
                    );
                }
            }
        }
        ...
        $actionObj->message = ($confirmnot == 1)
            ? 'Invoice confirmed & all dispatch notes updated'
            : 'Invoice rejected';
        $actionObj->type = 'success';
