	...
	public function Approveissue()
	{
		...
		if (!empty($tableData) && is_array($tableData)) {
			...
			foreach ($tableData as $rowtabledata) {
				...
				$stockid      = (int) ($rowtabledata['stockid'] ?? 0);
				$issueqty     = (float) ($rowtabledata['qty'] ?? 0);
				$account_id   = (int) ($rowtabledata['account_id'] ?? 0);
				$account_type = (int) ($rowtabledata['account_type'] ?? 0);

				if ($stockid <= 0 || $issueqty <= 0) {
				...
				$currentQuantity = 0;
				if ($query->num_rows() > 0) {
					$currentQuantity = (float) $query->row()->qty;
				}
				...
				if (!empty($accountData)) {
					...
					$this->db->update('tbl_print_issuedetail', $dataDetail);
				}
			}
		}
