...
class Invoiceinfo extends CI_Model{
    ...
    public function GetCustomerDetails($customer_id) {
        $this->db->select('`idtbl_customer`, `customer`, `nic`, `ref_no`, `bus_reg_no`, `nbt_no`, `svat_no`, `telephone_no`, `fax_no`, `address_line1`, `address_line2`, `city`, `state`, `delivery_address_line1`, `delivery_address_line2`, `delivery_city`, `delivery_state`, `business_status`, `payment_method`, `postal_code`, `vat_no`, `vat_customer`, `imagepath`, `country`, `tbl_company_idtbl_company`, `tbl_company_branch_idtbl_company_branch`, `status`, `insertdatetime`, `updatedatetime`, `tbl_user_idtbl_user`, `tbl_account_idtbl_account`');
        $this->db->from('tbl_customer');
        $this->db->where('idtbl_customer', $customer_id);
        $this->db->where('status', 1);
    ...
    public function Invoiceinsertupdate(){
		...
		if ($customerDetails && $customerDetails->vat_customer == 1) {
			...
			$tax_invoice_num = 'TAX' . $yearDigit . $taxCountPrefix;
		}
        ...
        try {
            ...
            if ($confirmnot == 1) {
                ...
                if ($finishJob == 1) {
                    ...
                    $this->db->update('tbl_customerinquiry_detail', [
                        'job_finish_status' => 1
                    ]);
                }

                $this->db->select('tbl_print_dispatch_idtbl_print_dispatch');
                $this->db->from('tbl_print_invoicedetail');
                $this->db->where('tbl_print_invoice_idtbl_print_invoice', $recordID);
                $this->db->where('status', 1);

                $dispatchDetails = $this->db->get()->result();


                if (!empty($dispatchDetails)) {

                    foreach ($dispatchDetails as $row) {

                        if (!empty($row->tbl_print_dispatch_idtbl_print_dispatch)) {
                            ...
                            $this->db->where(
                                ...
                                $row->tbl_print_dispatch_idtbl_print_dispatch
                            );
    ...
    public function Invoicecheckstatus() {
		$this->db->trans_begin();
