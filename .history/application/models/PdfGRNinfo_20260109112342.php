...
class PdfGRNinfo extends CI_Model {
    public function pdfgrnget($x) {
        ...
        $this->db->select('*, COALESCE(tbl_print_grn.idtbl_print_grn, 0) AS idtbl_print_grn, COALESCE(tbl_print_grn.total, 0) AS grn_total, COALESCE(tbl_print_grn.discount, 0) AS discount, COALESCE(tbl_print_grndetail.qty, 0) AS qty, COALESCE(tbl_print_grndetail.unitprice, 0) AS unitprice, , COALESCE(tbl_print_grndetail.tbl_material_group_idtbl_material_group, 0) AS tbl_material_group_idtbl_material_group');
        ...
        foreach ($query->result() as $rowlist) {
            ...
            if ($rowlist->tbl_material_group_idtbl_material_group == 4) {
