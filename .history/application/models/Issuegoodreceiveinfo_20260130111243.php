...
class Issuegoodreceiveinfo extends CI_Model{
	...
	public function Getunitpricefrombatch() {
		...
		$this->db->from('tbl_print_stock');
		$this->db->where('status', 1);
		$this->db->where_in('idtbl_print_stock', $batchno);
		$this->db->where('tbl_print_material_info_idtbl_print_material_info', $recordID);
...
public function Approveissue()
{
    ...
    /* ===============================
       UPDATE ISSUE APPROVE STATUS
       =============================== */
    $issueUpdateData = array(
        'approvestatus'  => 1,
        'updateuser'     => $userID,
        'updatedatetime' => $updatedatetime
    );
    ...
    $this->db->update('tbl_print_issue', $issueUpdateData);
    ...
    if (!empty($tableData) && is_array($tableData)) {
        ...
        foreach ($tableData as $rowtabledata) {
            ...
            /* --------- SAFETY CASTING --------- */
            $issueqty = is_numeric($issueqty) ? (float)$issueqty : 0;
            ...
            $currentQuantity = 0;
            if ($query->num_rows() > 0) {
                $currentQuantity = is_numeric($query->row()->qty)
                    ? (float)$query->row()->qty
                    : 0;
            }

            /* --------- PREVENT NEGATIVE STOCK --------- */
            if ($issueqty > $currentQuantity) {
                $issueqty = $currentQuantity;
            }
            ...
            $this->db->where('idtbl_print_stock', $stockid);
            $this->db->update('tbl_print_stock', $stockUpdateData);

            /* --------- UPDATE ISSUE DETAIL ACCOUNT --------- */
            $accountData = array();
            if ($account_type == 1) {
            ...
            $issuedetailUpdateData = array_merge(array(
                'updateuser'     => $userID,
                'updatedatetime' => $updatedatetime
            ), $accountData);
            ...
            $this->db->update('tbl_print_issuedetail', $issuedetailUpdateData);
        }
    }
