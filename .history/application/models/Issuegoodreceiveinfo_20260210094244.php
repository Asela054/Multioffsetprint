...
public function Approvestatus()
{
    ...
    $tableData    = $this->input->post('tableData') ?? [];
    $viewissueid  = $this->input->post('viewissueid') ?? null;

    if (!$viewissueid) {
        $this->db->trans_rollback();
        echo json_encode([
            'status' => 0,
            'message' => 'View Issue ID not provided.'
        ]);
        return;
    }
    ...
    try {
        ...
        if ($confirmnot == 1 && is_array($tableData) && count($tableData) > 0) {
            foreach ($tableData as $row) {
                ...
                if ($stockid) {
                    ...
                    $this->db->where('idtbl_print_stock', $stockid);
                    $this->db->update('tbl_print_stock', $dataStock);
                }
            }

            // Note: API call skipped since accounts are removed
        }
        ...
        $this->db->trans_rollback();
        error_log("ApproveStatus Error: " . $e->getMessage());
